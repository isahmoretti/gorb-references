...
const generateReference = (values) => {
    ...
    return (
        <span>
            ...
            {publishingCompany ? <>{<><>{local}</>: <>{publishingCompany}, </></>}</> : `${local}. `}
...
const Ebook = ({ back }) => {
    ...
    const handleSubmit = (values) => {
        setState((prev) => ({
            ...
            citationWithAuthor: generateCitationWithAuthor(values.responsibleName, values.yearOfPublication),
            citation: generateCitationWithoutAuthor(values.responsibleName, values.yearOfPublication)
        }));
